<script lang="ts">
  ...
  {:else}
    ...
    >
      <div class="w-3/4 text-center mx-auto p-6 bg-white rounded-lg bg-opacity-50" data-aos="fade-in-left">
        <h1
  ...
  {#if modalOpen}
    <div class="fixed top-0 left-0 w-full h-full bg-black bg-opacity-70 flex justify-center items-center z-50">
      <div class="relative" in:fade={{ duration: 300 }}>
        <img src={modalImageUrl} alt={modalImageAlt} class="max-w-full max-h-full rounded-lg" />
        <button
